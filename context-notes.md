...
- `vision.timeout_seconds=200`은 Vision API 1회 호출의 네트워크 응답 제한으로 유지한다. 이 값은 서버 불안이나 단일 요청 정체를 끊는 용도다.
- GUI에는 별도의 무진행 감지를 둔다. subprocess stdout에서 8분 동안 새 진행 신호가 없으면 멈춘 상태로 보고 프로세스를 종료한다.
- 8분 기준은 200초 API 타임아웃, 재시도 대기, 페이지 전처리 시간을 정상 범위로 허용하기 위한 값이다.
