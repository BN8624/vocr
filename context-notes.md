...
- 재실행 중 발견된 삼성 상호명 OCR 자동 교정 실험 변경은 정책과 맞지 않아 되돌렸다. 상호명은 자동 치환하지 않고 `merchant_correction_candidate` 후보로만 표시한다.
...
- 최신 커밋 `7eb140b 전체 카드 재검증 결과 보강`은 `origin/main`에 push했다. 이후 상호명 자동 교정 실험 변경도 되돌려 현재 작업트리는 깨끗하다.
