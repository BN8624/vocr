...
`review.html`의 첫 화면에는 `변환 확인` 순서표와 현재 단계 하나만 먼저 보입니다. 기본 흐름은 열 매핑 -> 검산/확인필요 -> Excel -> 고급 정보입니다. `PC에 매핑 저장`이나 `검산 기준 저장`을 누르면 다음 단계로 자동 이동합니다. 원본 확인은 매핑/확인필요 카드 안의 직접 링크로만 제공하고, 페이지 전체/청크 전체를 훑는 화면은 일반 검토 흐름에서 제외합니다.
