...
표 시작/끝이나 합계 영역이 잘린 페이지는 `review.html`의 각 페이지 카드에서 `이 페이지 자르기 조정`을 열어 비율을 저장할 수 있습니다. `serve_review.py`로 열어 둔 상태에서 저장하면 현재 output의 `merged/page_crop_profile.json`에 기록됩니다.
