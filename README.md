...
`review.html`의 첫 화면에는 `변환 확인` 순서표와 현재 단계 하나만 먼저 보입니다. 사용자의 판단이 필요한 단계만 표시하므로, 애매한 열이 없으면 열 매핑이 숨겨지고 검산/확인필요가 없으면 바로 Excel만 보입니다. `PC에 매핑 저장`이나 `검산 기준 저장`을 누르면 다음에 필요한 단계로 자동 이동합니다. 원본 확인은 매핑/확인필요 카드 안의 직접 링크로만 제공합니다.
