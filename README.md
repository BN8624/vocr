...
`review.html`의 첫 화면에는 `변환 확인` 순서표와 현재 단계 하나만 먼저 보입니다. 기본 흐름은 열 매핑 -> 검산/확인필요 -> Excel -> 원본 위치 -> 고급 정보입니다. `PC에 매핑 저장`이나 `검산 기준 저장`을 누르면 다음 단계로 자동 이동합니다. 원본 페이지, 청크, JSON 같은 기술 정보는 필요할 때만 뒤쪽 단계에서 확인하면 됩니다.
